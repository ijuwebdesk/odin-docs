...
<Aside type="tip">
**Can't paste the key?** Click the **detach** button to pop Odin out into its own window, then paste. If it still won't accept the key, paste it into a plain notepad first — copying by hand can pick up an invisible extra space or character that Odin rejects. Always use OpenRouter's **copy** button rather than selecting the text manually.
</Aside>
