...
<Aside type="note">
The SmartScreen warning appears because Odin's Windows installer is not yet signed with a trusted certificate. A code-signing certificate is in progress and will be available within the next week, after which the warning will no longer appear. The installer is safe to run.
</Aside>
